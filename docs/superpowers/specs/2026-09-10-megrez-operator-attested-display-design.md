...
restarts. They then execute three nonce-bound interaction cycles. Every cycle
requires physical evdev key presses, relative mouse movement, a complete left
click, trusted Firefox DOM events, the cyan state, and a hash-bound guest PNG
transferred over serial.
...
- Operator mode prints one bounded prompt asking the operator to confirm that
  the physical monitor shows the cyan cycle-three PASS page. The exact response
...
`PhysicalGraphicsResult` becomes schema version 2 and carries exactly one of:
...
Tests must prove CLI mutual exclusion, exact confirmation syntax and nonce
binding, deadline/EOF/duplicate rejection, three-cycle lifecycle ordering,
terminal-state and recovery preservation, result schema and pass invariants,
private publication, hash coverage, absence of fake HDMI files, and unchanged
external-capture behavior. The complete physical graphics, desktop, stability,
and probe suites run in the existing persistent container without downloads.
