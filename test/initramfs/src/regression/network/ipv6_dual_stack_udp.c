...
int main(void)
{
	...
	fprintf(stderr, "ASTERINAS_IPV6_DUAL_STACK_UDP_STAGE socket=%d errno=%d\n", listener, errno);
	if (listener < 0 ||
	...
	fprintf(stderr, "ASTERINAS_IPV6_DUAL_STACK_UDP_STAGE sender=%d errno=%d\n", sender, errno);
	if (sender < 0)
		goto fail;
	destination.sin_port = wildcard.sin6_port;
	sent = sendto(sender, request, sizeof(request), 0,
		      (struct sockaddr *)&destination, sizeof(destination));
	fprintf(stderr, "ASTERINAS_IPV6_DUAL_STACK_UDP_STAGE sent=%zd errno=%d\n", sent, errno);
	if (sent != sizeof(request) ||
	    getsockname(sender, (struct sockaddr *)&sender_local, &sender_local_len) < 0)
		goto fail;
	fprintf(stderr, "ASTERINAS_IPV6_DUAL_STACK_UDP_STAGE local-port=%u listener-port=%u\n",
	...
	fprintf(stderr, "ASTERINAS_IPV6_DUAL_STACK_UDP_STAGE received=%zd errno=%d\n", received,
			errno);
	...
	if (sendto(listener, response, sizeof(response), 0,
		   (struct sockaddr *)&peer, peer_len) != sizeof(response) ||
	    recvfrom(sender, buffer, sizeof(buffer), 0,
		      (struct sockaddr *)&sender_peer, &sender_peer_len) != sizeof(response) ||
